---
title: "neutrophil_timepoint_tf_ligandReceptor_analysis"
...
# lung neutrophils
load('/nfs/turbo/umms-csriram/scampit/Ryan/Projects/single-cell-neutrophil-flux-analysis/data/lung.neut.Robj')
...
# 2. Modify neutrophil and lung metadata PRN
```{r}

# Add disease vs healthy category
neut$dxHl <- ifelse(grepl('\\.h\\.', neut$sample), 'Hlthy', 'Dx')
lung$dxHl <- ifelse(grepl('\\.h\\.', lung$sample), 'Hlthy', 'Dx')


# Comparison category
neut$comp <- ifelse(neut$dxHl == 'Hlthy', 'Hlthy', neut$sample)
neut$comp <- factor(neut$comp, levels = c('Hlthy', 'lung.d.d7', 'lung.d.d14', 'lung.d.d21'))
...
plotTargetExpression <- function(seurat_obj = neut, 
                                 group_by = 'comp', 
                                 genes = NULL,
                                 col_ord = NULL){
...
Idents(neut) <- neut$comp
DotPlot(neut, features = nichenet_output$top_receptors %>% rev(), group.by = "comp", dot.min = .1) + RotatedAxis()

# plot expression of targets
plotTargetExpression(seurat_obj = neut, group_by = 'comp', genes = nichenet_output$top_targets, 
...
## 5G. Nichenet - Day 21 disease vs all other neutrophils
```{r}

# new metadata category for comparison
neut$all21 <- ifelse(neut$sample == 'lung.d.d21', 'lung.d.d21', 'allOtherNeut')
...
nichenet_output = nichenet_seuratobj_aggregate(
  seurat_obj = neut, 
  receiver = "Neutrophils", 
  condition_colname = "all21", condition_oi = "lung.d.d21", condition_reference = "allOtherNeut", 
  sender = "undefined", 
  ligand_target_matrix = ligand_target_matrix, lr_network = lr_network, weighted_networks = weighted_networks, organism = "mouse")

```
## Day 21 dx vs all other neutrophils Associated figures
```{r}
# plot target heatmap
nichenet_output$ligand_activity_target_heatmap
